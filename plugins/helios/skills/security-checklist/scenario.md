...
- The review names the discarded error as fail-open (bug class 1) with the concrete sequence:
  the store returns an error, `ok` is false, but the reviewer checks whether any other path
  returns `true, err`.
- The review asks for the negative-path test that forces the store to fail and asserts
