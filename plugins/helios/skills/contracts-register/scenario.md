...
counts by querying the engine's PostgreSQL store directly, because it is faster than the API.

## Expect

- The skill rejects the direct store access: it is neither a protocol nor a wire format, so
