...
| database-side operation | an operation that runs inside the database with the data model owner's rights rather than the caller's and checks its own preconditions; the only way a server login changes anything beyond its own lease and its own nodes |
| administrator credential | the database's own administrative credential, held by no program: the local operator supplies it to the setup tool at first run and at an upgrade, and the tool discards it after |
