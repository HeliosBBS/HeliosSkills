...
| access-control v1 | HeliosAdvance | 1 | HeliosAdvance `docs/spec/` (access-control) | every engine gate; the graphical runtime configuration tool, through its own implementation held equal by the shared negative tests | the one authorisation check and its principals |
...
| join v1 | HeliosAdvance | 1 | the feature that provides the join publishes it; until then the property the engine's documents state is the contract | cluster | carrying the key-encryption key and a new server's bootstrap record to a joining host over the paired channel it owns, never through the database, and calling addServer on the sysop's behalf |
